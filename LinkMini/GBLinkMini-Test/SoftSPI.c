 ...
 *   THIS IS THE VERSION FOR Tiny2313
 *
...
void softspi_setup(){
	...
	//Tiny2313: using INT0 on pin D2
	//set MCUCR bits to "Any logical change on INT0 generates an interrupt request."
	MCUCR = (MCUCR & 0xfc) | ((0 << ISC01) | (1 << ISC00));

	//enable interrupt INT0 (set bit 6 = INT0 to enabled)
	GIMSK |= 0x40;
